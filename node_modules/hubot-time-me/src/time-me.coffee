...
module.exports = (robot) ->
  ...
  robot.respond /time me ([\w\-]+)/i, (msg) ->
    ...
    if user? and robot.brain.data.locations[userId(robot, user)]?
      ...
      msg.send "Sorry, I don't know where the user #{user} is so I can't tell what time zone he or she is in. :( Try write the name with the first letter uppercase."

  robot.respond /([\w\-]+) is in (.*)/i, (msg) ->
    user =  msg.match[1].lower()
  ...
  robot.respond /where is ([\w\-]+)/i, (msg) ->
    ...
    if location?
      ...
      msg.send "Sorry, no idea where #{user} is. Try write the name with the first letter uppercase."
